...
<div id="page-inner">
  <div class="row">
    <div class="col-md-12">
		...
		<tr>
		<td align='left'>
      <h2><a href="?mnu=prediksi">Exponential Smoothing IN</a></h2>
      <h5>Penghalusan Eksponensial : Ft = Ft – 1 + α (Dt-1 – Ft-1)</h5>
	  <td align='right'>
      <h2><a href="?mnu=prediksi2">Moving Average IN</a></h2>
...
$ket2="Moving $alpha, Jumlah Prediksi : $jumlah Bulan";
...
elseif(strlen($moving)<1){
    echo "<script>alert('Silakan Input Moving Average... !');document.location.href='?mnu=prediksi';</script>";
        ...
        $gab= "<h2>Prediksi $jumlah Bulan YAD Thd $bahan ($id_bahan) <b>(Exponential Smoothing)<b></h2>";
...
echo "<h2>Prediksi $jumlah Bulan YAD Thd $bahan ($id_bahan) Moving Average </h2>";
$gab="<table border='1'>";
$hasil="";
$gab.="<tr><td>No<td>Periode<td>Formula<td>Prediksi</td></tr>";
    for($i=0;$i<$jumlah+$moving;$i++){
        $no=$i+1;
        $jum=$arH[$i];//getJumlah($conn,$ar1[$i],$ar2[$i],$id_bahan);
        $jums=$arS[$i];
        if($i<$moving){}
