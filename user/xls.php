...
if ($conn->connect_error) {
  trigger_error('Database connection failed: '  . $conn->connect_error, E_USER_ERROR);
    ...
    $buffer = "id_user".$separator ."nama_user".$separator ."alamat".$separator. "telepon".$separator ."email".$separator."username".$separator."password".$separator."status".$separator."keterangan".$separator; 
    $buffer .= $newline; 
    
  $sql="select `id_user`,`nama_user`,`alamat`,`telepon`,`email`,`username`,`password`,`status`,`keterangan` from `$tbuser` order by `id_user` desc";
  $jum=getJum($conn,$sql);	
  if($jum>0){						
